...
results = results.pivot_table(values='count', index='magnus_result', columns='weekday')

results_proportion = results.apply(lambda x: x/sum(x))


days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
results_proportion = round(results_proportion[days], 2)*100 #sort by days of week and convert to percent
...
for result in ['wins', 'draws', 'losses']:
    ...
    fig.add_trace(go.Bar(x=days, 
                         ...
                         name=result,
                         text=(results_proportion.loc[result,:]).apply(lambda x: str(int(x))) + '%',
                         textposition='inside',
                         textfont={'size':24},
...
fig.update_layout(height=900, width=1500,
                  ...
                  legend=dict(xanchor='center', x=0.5, y=1.1, font_size=25, orientation='h'))
