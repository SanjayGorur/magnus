...
def parse_data(text):
    ...
    metadata = list(
        map(lambda x: list(map(lambda y: y.split(':')[1], x)), metadata))
    ...
    moves_data = list(map(lambda x: re.sub(
        '\d+\.', '', x).replace('\n', ' '), moves_data))
    # choose first five moves
    moves_data = list(map(lambda x: x.split()[0:5], moves_data))
...
def preprocess_data(data):
    ...
    data.insert(7, 'magnus_color',
                data[['white', 'black']].apply(magnus_color, axis=1))
    data.insert(8, 'opponent', data[['white', 'black']].apply(
        magnus_opponent, axis=1))
    data.insert(10, 'magnus_result',
                data[['white', 'result']].apply(magnus_result, axis=1))
...
def prepare_data_to_viz(data):
    ...
    moves['counts'] = 1
    results = moves.groupby(['ids', 'result'], as_index=False)[
        'counts'].count()
    results = results.pivot_table(
        values='counts', columns='result', index='ids', fill_value=0)
    results['id'] = results.index

    moves['values'] = 1
    moves_count = moves.groupby('ids', as_index=False).agg({'values': 'count'})
    moves = moves.drop('values', axis=1).drop_duplicates(
